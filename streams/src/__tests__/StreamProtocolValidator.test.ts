...
it('rejects unknown event types', () => {
    ...
    expect(() => validator.check({ t: 'unknown_type' } as any)).toThrow('Stream validation failed for (test), unknown event type');
});
it('prevents events after done', () => {
    const validator = new StreamProtocolValidator('test');
    validator.check({ t: c_done });
    expect(() => validator.check({ t: c_item, item: 'test' })).toThrow('Stream validation failed for (test), got message after the stream is closed');
});
