...
export class Stream<ItemType = any> implements EventReceiver {
    ...
    pipe<T = any>(receiver: LooseEventReceiver<T>) {
        ...
        if (!receiver.event)
            throw new UsageError("invalid StreamReceiver, missing .putEvent");
        this.receiver = receiver;
        if (StreamSuperTrace) {
            console.log(`${this.getDebugLabel()} is now sending to:`, (receiver as any).getDebugLabel ? (receiver as any).getDebugLabel() : 'anonymous receiver');
        }
        if (this.backlog) {
